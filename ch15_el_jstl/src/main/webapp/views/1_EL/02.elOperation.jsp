...
<body>
	...
	<!-- integer.parseint는 String 타입만 형변환 할수있어서 Object타입을 형변환 못함, (int)는 일반자료형 변환 가능 -->
	<p/>
	...
	<h3>3. 동등 비교 연산</h3>
	sOne과 sTwo이 일치합니까? ${sOne == sTwo} 또는 ${sOne eq sTwo}<p/>
	sOne과 sTwo이 일치하지 않습니까? ${sOne != sTwo} 또는 ${sOne ne sTwo}<p/>
	
	big에 담긴값이 10과 일치합니까? ${big == small} 또는 ${big eq small}<p/>
	sOne에 담긴값이 "안녕"과 일치합니까? ${sOne == "안녕"} 또는 ${sOne eq "안녕"}<p/>
	<!-- el안에서 문자열 리터럴 제시시 홑따옴표든 쌍따옴표든 가리지 않는다 -->
	
	<h3>4. 객체가 null인지 또는 리스트가 비어있는지 비교</h3>
	pOne이 null입니까? ${pOne == null} 또는 ${empty pOne}<p/>
	pTwo이 null입니까? ${pTwo == null} 또는 ${empty pTwo}<p/>
	pOne이 null이 아닙니까? ${pOne != null} 또는 ${not empty pOne}<p/>
	
	aOne이 텅비어있습니까? ${empty aOne}, ${aOne == null}<p/>
	aOne이 텅비어있습니까? ${empty aTwo}<p/>
	
	<h3>5. 논리연산자</h3>
	and 연산자 : ${true && false} 또는 ${true and false}<p/>
	or 연산자 : ${true || false} 또는 ${true or false}<p/>
	
	big이 small보다 크고 aOne이 텅 비어있습니까? ${big gt small and empty aOne}<p/>
</body>
</html>