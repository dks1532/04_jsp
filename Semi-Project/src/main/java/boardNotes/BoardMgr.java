...
public class BoardMgr {
	...
	private static final String SAVEFOLDER = "D:\\ayh\\04_jsp\\Semi-Project\\src\\main\\webapp\\fileupload";
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			*/
			...
			sql = "insert into boardNotes values(SEQ_boardNotes.NEXTVAL,?,?,?,0,SEQ_boardNotes.CURRVAL,0,SYSDATE,?,?,0,?,?)";
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			if(keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(num) from boardNotes";
				pstmt = con.prepareStatement(sql);
			} else {
				sql = "select count(num) from boardNotes where " + keyField + " like ?";
				pstmt = con.prepareStatement(sql);
	...
	public ArrayList<BoardBean> getBoardList(String keyField, String keyWord, int start, int end) {
		...
		try {
			con = pool.getConnection();
			if(keyWord.equals("null") || keyWord.equals("")) {
				sql = "select BT2.* FROM(select rownum R1, BT1.* from(select * from boardNotes order by ref desc, pos)BT1)BT2 WHERE R1 >= ? AND R1 <= ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			} else {
				sql = "select BT2.* FROM(select rownum R1, BT1.* from(select * from boardNotes order by ref desc, pos)BT1 where " +keyField+ " like ?)BT2 WHERE R1 >= ? AND R1 <= ?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%" + keyWord + "%");
	...
	public void upCount(int num) {
		try {
			con = pool.getConnection();
			sql = "update boardNotes set count = count + 1 where num = " + num;
			pstmt = con.prepareStatement(sql);
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public BoardBean getBoard(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select * from boardNotes where num = " + num;
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			if(rs.next()) {
	...
	public void updateBoard(BoardBean upBean) {
		try {
			con = pool.getConnection();
			sql = "update boardNotes set name = ?, subject = ?, content = ? where num = ?";
			pstmt = con.prepareStatement(sql);
	...
	public boolean deleteBoard(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select count(*) from boardNotes where ref = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if(rs.next()) {
				...
				if(result == 1) {
					flag = true;
					sql = "select filename from boardNotes where num = ?"; // 파일이 있나 없나 확인
					pstmt = con.prepareStatement(sql);
					pstmt.setInt(1, num);
					rs = pstmt.executeQuery();
					...
					sql="delete from boardNotes where num = ?";
					pstmt = con.prepareStatement(sql);
					pstmt.setInt(1, num);
					pstmt.executeQuery();
				}
	...
	public void replyUpBoard(int ref, int pos) {
		try {
			con = pool.getConnection();
			sql = "update boardNotes set pos = pos + 1 where ref = ? and pos > ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
			pstmt.setInt(2, pos);
	...
	public void replyBoard(BoardBean bean) {
		try {
			con = pool.getConnection();
			sql = "insert into boardNotes values(SEQ_boardNotes.NEXTVAL,?,?,?,?,?,?,SYSDATE,?,?,0,NULL,NULL)";
	...
	public static void main(String[] args) {
		new boardNotesMgr().insert();
	}
	
	public void insert() {
		try {
			con = pool.getConnection();
			sql = "insert into boardNotes(num,name,subject,content,pos,ref,depth,regdate,count,pass)"
